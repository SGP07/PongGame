...
class Ball:
    ...
    def reset(self):
        self.x, self.y = self.original_x, self.original_y
        self.y_vel = 0
        # self.x_vel *= -1 #activate to change who gets the ball after a point 
...
def main():
    ...
    while run:
        ...
        if left_score >=WINNING_SCORE:
            won = True
            win_text = f"Left player won {left_score}, {right_score}"
        elif right_score >= WINNING_SCORE:
            won = True
            win_text = f"Right player won !! {left_score}, {right_score}"
